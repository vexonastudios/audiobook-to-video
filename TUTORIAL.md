...
- Transparent outer padding is removed automatically from PNG cover images. This lets a cover or 3D book mockup fill the cover area according to the visible artwork rather than the full transparent canvas.
- Use the sliders on the left to adjust background blur and opacity.
- **Cover Backlight** adds a soft, accent-colored glow behind the lower-left of the book, above the background image. Adjust its strength or set it to 0% to switch it off. **Cover Border Thickness** follows the visible book outline, including transparent 3D mockups. Both settings are saved with the project and apply to preview, opening titles, transitions, and export.
...
- Enable **Opening Title Sequence** to rotate the book title/subtitle, optional series and book number, author, original publication, and site through the right-side title area at the start. The small heading above the title is editable. An optional **Author photo** can be uploaded beneath the author field; it is cropped into a circle above the author’s name. Use **Horizontal focus**, **Vertical focus**, and **Zoom** to position the face correctly, and inspect the author card in Preview as you adjust it. Crop settings are saved with the project. Leave the photo empty to keep the original text-only author card. The title appears first, then the subtitle fades in two seconds later without moving the title. Both stay fully visible for at least four seconds. A full title/subtitle opening lasts eight seconds; a title alone lasts six. Other cards last about three seconds. Short opening chapters can shorten the holds, but skip the sequence if its minimum reading times cannot fit. Site addresses use smaller uppercase lettering. The cover remains fixed and chapter/audio timing does not change. The app renders each state once and lets FFmpeg animate the crossfades efficiently.
...
Once your fields are filled and chapters are parsed, click **Render Video**. The optimized renderer stores one high-resolution still per chapter, creates additional artwork only for transitions, intros, or the short promotional card, and then builds a broadly compatible constant-30fps video. Compatible compressed audio is copied directly, while other audio is cached after its first conversion. After the video is complete, the app automatically creates a separate 128 kbps MP3 beside it using the same base filename. The MP3 is encoded directly from the original audiobook source for the best available quality; an optional video intro's audio remains video-only. If that filename would overwrite the selected source MP3, the new file receives a `-128kbps` suffix instead. You can track progress in the log box at the bottom.
